...
3. **BLOCKER scan rule**: Scan the reviewer output for (a) lines starting with `BLOCKER:` and (b) a `REVIEW VERDICT: CRITICAL` line. If **neither** is present, treat the review as **approved** regardless of the `REVIEW RESULT` header — the loop is done, proceed to next pipeline step. A `REVIEW VERDICT: SHIP` line (with no `BLOCKER:` lines) is approved. `WARNING:` and `SUGGESTION:` lines are informational only and do not trigger a fix round. (The `REVIEW VERDICT:` line is the authoritative trigger where reviewers emit it — it subsumes both the `BLOCKER:`-prefix rule and the `EXIT_CONDITION_SCORE < 7` rule, closing the gap where a score-driven `CHANGES REQUIRED` carried no `BLOCKER:` line; see `CLAUDE/skills/reviewer-critical-ship-labels.md`.)
4. If one or more `BLOCKER:` lines are found **or** the reviewer wrote `REVIEW VERDICT: CRITICAL`, and iterations remain → log the fix request context to `orchestrator_log.md`, then run the fix role with the review remarks (include all BLOCKER lines), then run the reviewer again. The fix role is the member specified in the pipeline (e.g., idea_maker for ideation teams, coder for code teams).
...
| `<first_repo>/CLAUDE/skills/*.md` | Reusable skill files (zero or more) created in Step 6 |

## Step 6: Skills Creation (MANDATORY reflection)

After the pipeline completes (and after writing all output files), reflect on the task and create reusable skills. **This step is mandatory** — the only acceptable outcomes are (a) one or more skill files written, or (b) a one-line justification logged explaining why no skill was warranted.

1. **Ensure the skills directory exists**: Run `mkdir -p <SKILLS_DIR>` where `<SKILLS_DIR>` is the path provided in the Environment section under "Skills directory". For multi-repo workspaces this is inside the **first repo's** `CLAUDE/skills/` folder. The shell runner pre-creates this for you, but re-running mkdir is safe and defensive.
2. **Review what was done** — look at the plan, coder output, and any patterns that emerged.
3. **Check existing skills** — read the Available Skills section to avoid duplicating what already exists.
4. **Run through the skill checklist** — at minimum consider whether any of these surfaced during the task:
   - (a) A new file type, file format, or component pattern.
...
- Create at most **3 new skills** per pipeline run (to avoid clutter).
- Only create skills for **genuinely reusable** patterns, not one-off tasks.
- Skills apply to **all** tasks, including those classified as simple — even a one-line typo fix can surface a non-obvious convention worth recording.

### Mandatory justification when no skill is created

If, after running the checklist, you determine no skill is warranted, append a one-line justification to `/workspace/.klodTalk/history/orchestrator_log.md` under a `## Step 6: Skills Creation` heading. Examples:

- `No skill created — task was a one-line typo fix; no reusable pattern surfaced.`
- `No skill created — pattern matches existing skill 'add-websocket-message.md'.`
- `No skill created — change was config-only and project-specific.`

This makes the no-skill decision explicit and auditable.

## Step 6.5: Commit New Skills

If one or more skill files were created or modified in Step 6, commit them so they are persisted with the codebase:

1. Determine the **first repo's path** from the Environment section. For single-repo workspaces this is `/workspace`. For multi-repo workspaces it is the first entry in the repos JSON.
2. From inside the first repo's directory, stage and commit the skills:
   ```bash
   cd <first_repo_path>
   git add CLAUDE/skills/
   # Verify there is something staged before committing
   if ! git diff --cached --quiet; then
     git commit -m "Add skills: <brief topic list>" -m "Generated by KlodTalk orchestrator after task: <one-line task summary>"
   fi
   ```
3. If no skills were created, **skip this step entirely** — do not produce an empty commit.
4. If the team definition specifies "No commits" in Special Rules, **skip the commit** but still write the skill files to disk (so they appear in the working tree for the human to commit later).
5. Record the commit (hash, message) — or the reason for skipping — in `orchestrator_log.md`.

Note on ordering: Step 6/6.5 deliberately run **after** the coder's final commit. Skills emerge from reflection on the entire pipeline, so committing them separately is cleaner than threading them through the coder role. The result is two commits per task when a skill is added: the coder's implementation commit, and the orchestrator's `Add skills: ...` commit.
