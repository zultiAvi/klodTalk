...
| coder | coder | opus | |
| test_runner | test_runner | sonnet | |
...
5. **test_runner** -- Run the project's test suite (`pytest tests/ -v`). Write results to `.klodTalk/team/current/test_runner_output.txt` with `TEST_RESULT: PASS` or `TEST_RESULT: FAIL`.
   - Test-fix loop: if `TEST_RESULT: FAIL`, send back to **coder** for fixes, then re-run tests.
   - Max iterations: **2**
6. **reviewer** -- Review the implementation for correctness, safety, and adherence to codebase conventions.
