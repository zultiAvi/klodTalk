...
summary: "Claude Code (>= 2.1.163) refuses to start if the CLI is outside [requiredMinimumVersion, requiredMaximumVersion] managed settings; pin a floor (currently 2.1.197, which makes Claude Sonnet 5 the CLI default — see sonnet-5-default-breaking-changes.md; the highest version any active KlodTalk hook/skill needs) to turn silent feature no-ops into a loud startup failure. Doc/config only — do NOT edit Dockerfile.agent."
---
...
- **2.1.197 (2026-07-01): Claude Sonnet 5 is now the CLI default** — the `sonnet` alias resolves to `claude-sonnet-5` in an unguarded CLI. Sonnet 5 has a 1M-token native context, 128k max output, and introductory $2/$10 per MTok pricing through Aug 31 2026. Breaking vs Sonnet 4.6: adaptive thinking is ON by default, manual `thinking: {budget_tokens}` → HTTP 400, non-default `temperature`/`top_p`/`top_k` → HTTP 400, and a ~30% tokenizer inflation. KlodTalk's `enforceAvailableModels: true` currently BLOCKS Sonnet 5 (it is intentionally NOT in `availableModels` pending a sampling-param audit — see `sonnet-5-default-breaking-changes.md`), so this bump is doc/floor hygiene, not a live enablement. Current recommended floor.
- **2.1.196 (2026-06-29): background-job transcript-deletion fix + streaming idle watchdog ON BY DEFAULT + MCP untrusted-workspace `.mcp.json` pending-approval hardening** — (1) waking a background job no longer permanently deletes its conversation and re-runs the original prompt when the transcript probe misreads a real transcript; the file is now set aside, never deleted — this directly protects KlodTalk's nightly pipeline that wakes background sessions, whose transcripts the `post_session_snapshot.sh` / `subagent_lifecycle_logger.sh` hooks depend on. (2) The streaming idle watchdog is now ON BY DEFAULT for all providers — it aborts and retries when a response stream produces no events for 5 minutes. KlodTalk's long-running Docker container sessions stream for minutes at a time during heavy multi-turn nightly runs; if a legitimate-but-slow stream is killed prematurely, set `CLAUDE_ENABLE_STREAM_WATCHDOG=0` in the Docker container env (the `run_agent.py` container env dict) to disable it (the idle threshold is configurable via `CLAUDE_STREAM_IDLE_TIMEOUT_MS`). (3) `claude mcp list`/`get` no longer auto-spawn `.mcp.json` servers that a repo self-approved via a committed `.claude/settings.json`; untrusted workspaces show `⏸ Pending approval` — a security hardening that affects KlodTalk agents working in cloned/mounted external repos (see `mcp-untrusted-workspace-pending-approval.md`). Also adds organization default model support (admins set it in the org console), readable default session names at start, clickable file attachments in chat (Cmd/Ctrl-click reveals the file in Finder/Explorer), and fixes PowerShell `git diff`/`git grep`, `egrep`/`fgrep`, and quoted `|` search patterns being reported as failures when they exit 1 (now matches Bash) — none KlodTalk-critical (headless containers).
- **2.1.195 (2026-06-26): three background-agent durability fixes + hyphenated hook-matcher exact-match** — (1) background jobs no longer disappear from `claude agents` or lose data when written by a newer Claude Code version (a mixed host/container CLI-version hazard for KlodTalk's Docker agents); (2) background agent daemons no longer run unreachable when the control socket fails to start, which previously blocked restarts; (3) reopening a crashed background task no longer shows a blank screen for up to 5 seconds before restarting. These three protect KlodTalk's long-lived nightly-pipeline/Docker agent sessions whose state the `post_session_snapshot.sh` / `subagent_lifecycle_logger.sh` hooks depend on (see `background-agent-durability.md`). Also fixes hook matchers with hyphenated identifiers (e.g. `code-reviewer`, `mcp__brave-search`) accidentally substring-matching — they now exact-match; use `mcp__brave-search__.*` to match all tools from a hyphenated MCP server (KlodTalk has zero hyphenated matchers today, so this is forward-looking, see `hyphenated-hook-matcher-exact-match.md`). Also adds `CLAUDE_CODE_DISABLE_MOUSE_CLICKS` and voice-dictation/`/plugin` fixes (not KlodTalk-relevant). v2.1.194 was internal-only (no separate user-facing changelog entry).
...
```json
{
  "requiredMinimumVersion": "2.1.197"
}
```
...
- `mcp-untrusted-workspace-pending-approval.md` — the 2.1.196 MCP untrusted-workspace `.mcp.json` pending-approval hardening this floor reflects.
- `sonnet-5-default-breaking-changes.md` — the v2.1.197 Claude Sonnet 5 default flip and its breaking changes this floor reflects.

## Source
- Claude Code CHANGELOG v2.1.163 / v2.1.169 / v2.1.170 / v2.1.172 / v2.1.174 / v2.1.176 / v2.1.178 / v2.1.179 / v2.1.181 / v2.1.183 / v2.1.185 / v2.1.186 / v2.1.187 / v2.1.191 / v2.1.193 / v2.1.195 / v2.1.196 / v2.1.197 —
